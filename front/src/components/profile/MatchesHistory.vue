<template>
	<div class="grid grid-cols-4 items-center pb-7">
		<div class="col-span-2 md:col-span-1 text-left flex flex-col">
			<p v-if='$store.getters.getId == match.winner.id' class="victory font-sans font-bold text-3xl">VICTORY</p>
		...
		<div class="hidden lg:flex col-span-2 md:col-span-1 justify-self-center">
			<div class="flex flex-rows justify-between text-3xl w-32">
				<p v-if="$store.getters.getId == match.player1.id" class="w-10 text-left">{{score1}}</p>
				<p v-else class="w-10 text-left">{{score2}}</p>
				<p class="font-medium">vs</p>
				<p v-if="$store.getters.getId == match.player1.id" class="w-10 text-right">{{score2}}</p>
				<p v-else class="w-10 text-right">{{score1}}</p>
			</div>
		</div>
		<div class="col-span-2 md:col-span-1 text-xl flex flex-rows justify-self-end">
			<p v-if="$store.getters.getId == match.player1.id" class=' pt-3 font-medium'>{{match.player2.nickname}}</p>
			<p v-else class=' pt-3 font-medium'>{{match.player1.nickname}}</p>
			<img v-if="$store.getters.getId == match.player1.id" class="image ml-4 h-14 w-14" :src="'http://localhost:3000/' + match.player2.avatarPath">
